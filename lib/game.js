...
class Game {
  ...
  getPossiblePositions(playerId, diceValue) {
    const player = this.players[playerId];
    const allPlayersPositions = this.players.map(player =>
      player.position in rooms ? '' : player.position
    );
    return getValidPositions(allPlayersPositions, player, diceValue);
  }
  ...
  movePlayer(playerId, location) {
    ...
    if (player.lastPosition in rooms) {
      this.addActivity(
        playerId,
        `has came out of ${rooms[player.lastPosition]}`
      );
    }
  ...
  isPlayersTurn(playerId) {
    ...
    if (isPlayersTurn && this.diceRollable) {
      this.players[playerId].changeMessage('Your turn, roll dice');
